...
void ESP8266Interface::stopServer()
{
	_esp.stopServer();
	_esp.reset();
}
...
{
	return _esp.startup(mode);
}

void ESP8266Interface::attach(Callback<void()> func)
{
	_esp.attach(func);
}
...
{
	return _esp.send(id, data, amount);
}

int32_t ESP8266Interface::recv(int id, void * data, uint32_t amount)
{
	return _esp.recv(id, data, amount);
}

bool ESP8266Interface::close(int id)
{
	return _esp.close(id);
}
